 ...
 return <section className="panel outreach-center">
  ...
  {!items.length?<div className="empty">No proposal actions are waiting. Generate a proposal from a qualified lead.</div>:items.map(item=><article className="outreach-row" key={item.id}>
   ...
   <div className="outreach-actions">
    {item.destination_url&&<button className="secondary" disabled={busy===item.id} onClick={()=>act(item.id,async()=>{await api.openOutreach(item.id);window.open(item.destination_url,"_blank","noopener,noreferrer")})}>{actionLabels[item.action_type]||"Open destination"} ↗</button>}
    <button className="secondary" disabled={busy===item.id} onClick={()=>copy(item.message)}>Copy proposal</button>
    {item.medium==="email"&&item.status==="approved"&&<button disabled={busy===item.id} onClick={()=>act(item.id,()=>api.sendOutreach(item.id))}>Send email</button>}
    {item.medium!=="email"&&<button disabled={busy===item.id} onClick={()=>act(item.id,()=>api.markOutreachSubmitted(item.id))}>Mark submitted</button>}
   </div>
  </article>)}
 </section>
}
