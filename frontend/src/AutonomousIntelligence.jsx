...
export default function AutonomousIntelligence(){
 ...
 useEffect(()=>{void load()},[]);
 async function execute(item){setBusy(true);try{await api.acquisitionAction(item.id,{action:item.recommended_action,approved:true,mode:"approval_required"});await load()}catch(e){setError(e.message);setBusy(false)}}
 return <section className="panel" style={{marginTop:20}}>
  ...
  <div className="card" style={{marginTop:16}}><div className="eyebrow">PHASE 18 · REVENUE OPTIMIZATION</div><h3>Strategy & channel attribution</h3>
   {!revenue?.by_strategy?.length?<div className="empty">Revenue attribution will appear as deal values are recorded.</div>:<div className="table-wrap"><table><thead><tr><th>Strategy</th><th>Leads</th><th>Revenue</th><th>Expected</th></tr></thead><tbody>{revenue.by_strategy.map((x,i)=><tr key={i}><td>{x.key||"unspecified"}</td><td>{x.leads}</td><td>{Number(x.revenue||0).toFixed(2)}</td><td>{Number(x.expected||0).toFixed(2)}</td></tr>)}</tbody></table></div>}
  </div>
 </section>
}