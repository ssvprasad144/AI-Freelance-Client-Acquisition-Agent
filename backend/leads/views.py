...
class LeadViewSet(viewsets.ModelViewSet):
    ...
    def proposal(self,request,pk=None):
        ...
        outreach=Outreach.objects.create(lead=lead,channel="email",message=message,status="draft")
...
def dashboard(request):
    return JsonResponse({"opportunities":Lead.objects.exclude(status="archived").count(),"qualified":Lead.objects.filter(status="qualified").count(),"proposals":Lead.objects.filter(status="proposal").count(),"replies":Lead.objects.filter(status="replied").count(),"high_match":LeadAnalysis.objects.filter(match_score__gte=80).count()})

def activity(request):
    return JsonResponse({"items":ActivityLogSerializer(ActivityLog.objects.all()[:50],many=True).data})
