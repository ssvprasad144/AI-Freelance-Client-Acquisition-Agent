...
def deterministic_analysis(lead) -> dict[str, Any]:
    ...
    score = min(95, 30 + len(hits) * 15)
    service = hits[0][0] if hits else "Needs review"
    projects = []
    for _, ps in hits:
        for p in ps:
    ...
    return {
        ...
        "recommended_approach": "Confirm requirements, define the smallest deliverable, then propose an implementation plan grounded in the existing project evidence.",
...
def generate_proposal(lead, analysis) -> str:
    ...
    if not settings.OPENAI_API_KEY:
        return (
            f"Hi,\n\nI came across your request for {lead.title}. "
            f"I work across {analysis.service_match or 'full-stack and AI development'} and can help structure the work around the requirements you listed. "
            f"Relevant project evidence: {projects or 'available projects in my portfolio'}.\n\n"
    ...
    prompt = {
        "profile": PROFILE,
        "lead": {"title": lead.title, "company": lead.company, "description": lead.description, "technologies": lead.technologies},
        "analysis": {
            "service_match": analysis.service_match,
            "matching_projects": analysis.matching_projects,
    ...
    response = client.responses.create(
        model=settings.OPENAI_MODEL,
        input=[
            {"role": "system", "content": "Draft a concise, professional freelance proposal. Never invent facts, clients, metrics, or integrations. Do not imply that any message has been sent."},
