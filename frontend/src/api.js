...
clients:async()=>collection(await request("/clients/")),
client:id=>request("/clients/"+id+"/"),
clientIntelligence:id=>request("/clients/"+id+"/intelligence/",{method:"POST"}),
meetings:async()=>collection(await request("/meetings/")),
createMeeting:data=>request("/meetings/create/",{method:"POST",body:JSON.stringify(data)}),
...
revenue:()=>request("/revenue/"),revenueRecords:async()=>collection(await request("/revenue/records/")),recordRevenue:data=>request("/revenue/record/",{method:"POST",body:JSON.stringify(data)}),
...
discoveryProfiles:()=>request("/discovery/profiles/"),
 ...
 proposalWorkspace:id=>request("/proposals/"+id+"/"),
 editProposal:(id,content)=>request("/proposals/"+id+"/edit/",{method:"PUT",body:JSON.stringify({content})}),
 reviseProposal:(id,instruction)=>request("/proposals/"+id+"/revise/",{method:"POST",body:JSON.stringify({instruction})}),
 restoreProposal:(id,version_number)=>request("/proposals/"+id+"/restore/",{method:"POST",body:JSON.stringify({version_number})}),
...
sendOutreach:id=>request("/outreach/"+id+"/send/",{method:"POST"}),sendFollowup:id=>request("/followups/"+id+"/send/",{method:"POST"}),
 ...
 createFollowupSequence:(id,data={})=>request("/leads/"+id+"/followup-sequence/",{method:"POST",body:JSON.stringify(data)}),
 followupSequences:async()=>collection(await request("/followup-sequences/")),
 cancelFollowupSequence:id=>request("/followup-sequences/"+id+"/cancel/",{method:"POST"}),
