...
def select_profile(ttl_hours,only_if_due=True,strategy_id=None):
    now=timezone.now(); stale=now-timezone.timedelta(hours=ttl_hours)
    if strategy_id:
        strategy=_strategy_map()[strategy_id]; profiles=DISCOVERY_PROFILES
    else:
        arm=_select_arm(); strategy_id=arm["strategy_id"]; strategy=_strategy_map()[strategy_id]
        profiles=[p for p in DISCOVERY_PROFILES if p["id"]==arm["profile_id"]]
    eligible=[]
    for p in profiles:
    ...
    if not eligible:
        candidates=[]
        for p in profiles:
            q=_strategy_query(p,strategy); searched=DiscoveryQueryCache.objects.filter(profile_id=p["id"],normalized_query=normalize_query(q)).values_list("searched_at",flat=True).first()
            candidates.append((searched or datetime.min.replace(tzinfo=now.tzinfo),p,q))
        _,p,q=min(candidates,key=lambda x:x[0])
    else: _,p,q=min(eligible,key=lambda x:x[0])
    return {**p,"strategy_id":strategy_id,"query":q,"selection_mode":"contextual-bandit" if not strategy_id else "adaptive"}
