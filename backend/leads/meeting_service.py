...
def sync_meeting_context(meeting):
    ...
    if lead and not meeting.client_id:
        ...
        meeting.contact=meeting.contact or contact
        meeting.save(update_fields=["client","contact","updated_at"])
    return meeting

def record_meeting_event(meeting,event_type):
    if meeting.lead_id:
        AcquisitionEvent.objects.create(
            lead=meeting.lead,
            event_type=event_type,
            source=meeting.lead.source,
            profile_id=meeting.lead.discovery_profile,
            strategy_id=meeting.lead.discovery_strategy,
            query=meeting.lead.discovery_query,
...
def apply_meeting_status(meeting,new_status):
    ...
    if new_status=="completed" and not meeting.completed_at:
