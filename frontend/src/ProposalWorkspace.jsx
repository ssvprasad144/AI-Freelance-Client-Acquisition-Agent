...
export default function ProposalWorkspace({proposalId,onClose,demoMode=false}){
 ...
 useEffect(()=>{if(import.meta.env.VITE_PUBLIC_PREVIEW!=="false")return;void load()},[proposalId]);
 ...
 return <section className="panel proposal-workspace">
  ...
  <div className="actions">
   <button disabled={demoMode||busy||!content.trim()} onClick={()=>act(()=>api.editProposal(proposalId,content))}>Save new version</button>
   <input value={instruction} onChange={e=>setInstruction(e.target.value)} placeholder="e.g. make it shorter and more technical"/>
   <button className="secondary" disabled={demoMode||busy||!instruction.trim()} onClick={()=>{const x=instruction;setInstruction("");return act(()=>api.reviseProposal(proposalId,x))}}>AI revise</button>
   <button className="secondary" onClick={onClose}>Close</button>
  </div>
  <div className="version-list">{(data.versions||[]).map(v=><div className="due-row" key={v.id}><span>v{v.version_number} · {v.source}</span><button className="secondary" disabled={demoMode||busy} onClick={()=>act(()=>api.restoreProposal(proposalId,v.version_number))}>Restore</button></div>)}</div>
 </section>
}
