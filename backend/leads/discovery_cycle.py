    ...
    with transaction.atomic():
        for item in items:
            ...
            Lead.objects.create(
                ...
                lead_type=item.get("lead_type", "freelance"),
...
def run_discovery_cycle(query=None, source="live", qualification_limit=None, profile_id=None):
    ...
    cache_fresh = bool(
        cache
        and cache.searched_at
        and cache.searched_at >= cycle_started - timezone.timedelta(
            hours=settings.DISCOVERY_QUERY_CACHE_TTL_HOURS
        )
    )
    if cache_fresh:
        payload = _payload(
            ...
            cached=True,
            searched=False,
            skip_reason="fresh query cache",
        )
        ActivityLog.objects.create(
            event_type="discovery.cache_hit",
            message="Discovery query served from freshness cache; no web search performed.",
            metadata=payload,
        )
        return payload
    ...
    if source == "live":
        if _daily_search_count() >= settings.DISCOVERY_MAX_SEARCHES_PER_DAY:
            payload = _payload(
                ...
                profile_id,
                cached=True,
                skip_reason="daily web-search budget exhausted",
            )
            ActivityLog.objects.create(
                event_type="discovery.skipped",
                message="Discovery search skipped: daily web-search budget exhausted.",
                metadata=payload,
            )
            return payload
        ...
        if _fresh_qualified_inventory() >= settings.DISCOVERY_TARGET_QUALIFIED_LEADS:
            payload = _payload(
                ...
                profile_id,
                cached=True,
                skip_reason="qualified lead inventory is already healthy",
            )
            ActivityLog.objects.create(
                event_type="discovery.skipped",
                message="Discovery search skipped: qualified lead inventory is already healthy.",
                metadata=payload,
            )
            return payload
        ...
        ActivityLog.objects.create(
            event_type="discovery.search",
            message="Live discovery web search started.",
            metadata={"profile_id": profile_id, "query": query},
        )

    result = DiscoveryService().discover(query, source)
    items = result.get("leads", [])

    # Rank results locally before spending crawler requests.
    ranked = []
    for item in items:
        if not item.get("title") or not item.get("description") or not item.get("source_url"):
            continue
        ...
        score = local_lead_score(preview)
    ...
    if settings.CRAWLER_ENABLED:
        crawl_candidates = [
            ...
            if score >= settings.CRAWLER_MIN_LEAD_SCORE
        ][: settings.CRAWLER_MAX_LEADS_PER_CYCLE]
        ...
        if crawl_candidates:
            ...
            items = [
                enriched_by_url.get(_normalize_url(item.get("source_url", "")), item)
                for item in items
            ]
    ...
    limit = min(
        qualification_limit or settings.DISCOVERY_MAX_RESULTS,
        settings.AI_QUALIFICATION_MAX_LEADS_PER_CYCLE,
    )
    now = timezone.now()
    candidates = list(
        Lead.objects.filter(
            ...
            discovered_at__gte=cycle_started,
        )
    ...
    payload = _payload(
        ...
        ai_calls=analyzed - locally_filtered,
