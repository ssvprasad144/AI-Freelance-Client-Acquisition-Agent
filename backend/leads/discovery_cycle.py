...
def _fresh_qualified_inventory():
    now=timezone.now(); return Lead.objects.filter(status="qualified").filter(models.Q(expires_at__isnull=True)|models.Q(expires_at__gt=now)).count()

def _daily_search_count(): return ActivityLog.objects.filter(event_type="discovery.search",created_at__date=timezone.localdate()).count()
...
def run_discovery_cycle(query=None,source="live",qualification_limit=None,profile_id=None,strategy_id=None):
    ...
    if not query:
        profile=select_profile(settings.DISCOVERY_QUERY_CACHE_TTL_HOURS, strategy_id=strategy_id); query,profile_id,strategy_id=profile["query"],profile["id"],profile.get("strategy_id")
    ...
    if source=="live":
        if _daily_search_count()>=settings.DISCOVERY_MAX_SEARCHES_PER_DAY:
            payload=_payload(query,profile_id,cached=True,skip_reason="daily web-search budget exhausted",strategy_id=strategy_id); ActivityLog.objects.create(event_type="discovery.skipped",message="Discovery search skipped: daily web-search budget exhausted.",metadata=payload); return payload
        if _fresh_qualified_inventory()>=settings.DISCOVERY_TARGET_QUALIFIED_LEADS:
            payload=_payload(query,profile_id,cached=True,skip_reason="qualified lead inventory is already healthy",strategy_id=strategy_id); ActivityLog.objects.create(event_type="discovery.skipped",message="Discovery search skipped: qualified lead inventory is already healthy.",metadata=payload); return payload
        ActivityLog.objects.create(event_type="discovery.search",message="Live discovery web search started.",metadata={"profile_id":profile_id,"strategy_id":strategy_id,"query":query})
    result=DiscoveryService().discover(query,source); items=result.get("leads",[]); raw_results=len(items); valid_results=sum(1 for item in items if item.get("title") and item.get("description") and item.get("source_url"))
    ranked=[]; seen=set()
    for item in items:
    ...
    DiscoveryQueryCache.objects.update_or_create(profile_id=profile_id,normalized_query=normalized,defaults={"query":query,"searched_at":timezone.now(),"result_count":len(items)})
    ...
    ActivityLog.objects.create(event_type="discovery.completed",message=f"Discovery cycle completed: {created} new leads, {qualified} qualified.",metadata=payload); return payload
